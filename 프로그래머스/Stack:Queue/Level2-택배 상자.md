...
택배 기사님이 원하는 상자 순서를 나타내는 정수 배열 order가 주어졌을 때, 영재가 몇 개의 상자를 실을 수 있는지 return 하는 solution 함수를 완성하세요.<br><br>

## 💡 해결 방법
문제를 읽고 컨테이너 벨트는 Queue(FIFO) 보조 컨테이너 벨트는 Stack(LIFO) 이란 생각이 들었습니다.
주 벨트에서 오는 상자를 검사하고 순서에 맞지 않으면 보조벨트(스택)에 있는 상자를 검사하여 해결하면 될 것이라고 생각했습니다.<br><br>
...
func solution(_ order:[Int]) -> Int {
    ...
    for i in 1...order.count {
        ...
        while !subBelt.isEmpty && order[count] == subBelt.last! {
            subBelt.removeLast()
            count += 1
        }
    }
